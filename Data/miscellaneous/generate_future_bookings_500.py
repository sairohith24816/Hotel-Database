...
def generate_future_bookings():
    ...
    for i in range(num_bookings):
        booking_id = f"B{last_number + i + 1:07d}"
    ...
    # Create DataFrame and sort by booking date, then by booking ID
    df = pd.DataFrame(future_bookings, columns=[
      ...
      # Sort by booking date first, then by booking ID to maintain order
    df['BookingDateSort'] = pd.to_datetime(df['BookingDate'], format='%d-%m-%Y')
    df = df.sort_values(['BookingDateSort', 'BookingID']).reset_index(drop=True)
    df = df.drop('BookingDateSort', axis=1)
    
    # Reassign BookingIDs to maintain sequential order after sorting
